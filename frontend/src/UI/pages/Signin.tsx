...
function Login() {
    ...
    const [idValid, setIdValid] = useState<Valid>({isValid: false, message:""});
    const [pwValid, setPwValid] = useState<Valid>({isValid: false, message:""});
    const [pwCheckValid, setPwCheckValid] = useState<Valid>({isValid: false, message:""});
    const [nickNameValid, setNickNameValid] = useState<Valid>({isValid: false, message:""});
    ...
    const pwBlurHandle = (e: React.FocusEvent<HTMLInputElement>) => {
        ...
        if (!pwRE.test(pw)) {
            setPwValid({ isValid: true, message: "4~12글자, 영문대소문자, 숫자, 특수문자(~,!,@)만 사용 가능합니다." });
            return;
        }
    ...
    const nickNameBlurHandle = (e: React.FocusEvent<HTMLInputElement>) => {
        ...
        if (!nickNameRE.test(nickName)) {
            setNickNameValid({ isValid: true, message: "2~12글자 사이로 입력해주세요." });
            return;
        }
            ...
            <Grid container spacing={3} justifyContent="center">
                ...
                <Grid item xs={8}>
                    <Box sx={{
                        ...
                        <button type="button" onClick={() => { clickSignin() }} >회원가입</button>
                    </Box>
                </Grid>
            </Grid>
