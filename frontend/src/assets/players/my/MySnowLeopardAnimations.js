...
export default function MySnowLeopardAnimations({
  destinationPoint,
  ...
  useEffect(() => {
    if (destinationPoint) {
      ...
      group.current.name = "snowleopard";
    }
  }, [destinationPoint]);
  useFrame((state) => {
    actions["Idle_A"].play();
      ...
      <group ref={group} dispose={null} position={[-30, 0, -30]}>
        <group name="Scene">
          <group name="Rig" scale={2}>
            ...
            <skinnedMesh
              name="Puffin"
              geometry={nodes.Puffin.geometry}
              material={materials.M_Puffin}
              skeleton={nodes.Puffin.skeleton}
              morphTargetDictionary={nodes.Puffin.morphTargetDictionary}
              morphTargetInfluences={nodes.Puffin.morphTargetInfluences}
              // 그림자 설정은 여기에!
              castShadow
            />
          </group>
...
useGLTF.preload("/Puffin_Animations.gltf");
